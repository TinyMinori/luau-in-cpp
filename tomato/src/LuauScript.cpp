...
namespace tomato {
    ...
    LuauScript::LuauScript(const fs::path scriptPath) {
        ...
        int result = luau_load(this->p_L, scriptPath.c_str(), bytecode, bytecodeSize, 0);
        free(bytecode);
    ...
    int     LuauScript::run() {
        ...
        if (lua_type(this->p_L, -1) == LUA_TSTRING)
            std::cerr << "\n\t" << lua_tostring(this->p_L, -1);
        else
            std::cerr << "Unknown error UwU";
